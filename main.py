    ...
    def __init__(self, master):
        ...
        try:
            ...
            while True:
                ...
                loopVal = loopVal + 1
                self.LoopLabel = Label(frame, text=('Loop ' + str(loopVal)), borderwidth=1).grid(row=2, column=2,padx=5, pady=5)
                pir, counter = M.getPir(pirSensor, board, counter, pirLight, buzzSensor)
                self.PirValue = Label(frame, text=(pir), borderwidth=1).grid(row=3, column=2, padx=5, pady=5)
                temp, far = M.getTemp()
                self.TempValue = Label(frame, text=(temp), borderwidth=1).grid(row=4, column=2, padx=5, pady=5)
                M.tempLight(far, board, ledRed, ledGreen, ledBlue, highTemp, lowTemp)
                light = M.getLight(lightSensor, board)
                self.LightValue = Label(frame, text=(light), borderwidth=1).grid(row=5, column=2, padx=5, pady=5)
                dist = M.getDist(dtSensor, deSensor, board)
                self.DistValue = Label(frame, text=(dist), borderwidth=1).grid(row=6, column=2, padx=5, pady=5)
                print (screen)
